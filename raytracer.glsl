...
#define root2 0.7071067811865475
...
mat3 BG_COORDS = ROT_Y(45.0 * conv);

// planet texture coordinate system
const float PLANET_AXIAL_TILT = 30.0 * conv;
mat3 PLANET_COORDS = ROT_Y(PLANET_AXIAL_TILT);

const float FOV_ANGLE_DEG = 90.0;
float FOV_MULT = 1.0 / tan(conv * FOV_ANGLE_DEG*0.5);
...
vec4 planet_intersection(vec3 old_pos, vec3 ray, float t, float dt,
    ...
    rot_phase = rot_phase * PLANET_ROTATION_ANG_VEL*0.5/pi;
...
void main() {
    ...
    for (int j=0; j < NSTEPS; j++) {
        ...
        step = MAX_REVOLUTIONS * 2.0*pi / float(NSTEPS);
